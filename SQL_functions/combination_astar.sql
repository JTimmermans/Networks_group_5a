DROP FUNCTION IF EXISTS combination_astar();
CREATE OR REPLACE FUNCTION combination_astar(IN xcoord1 double precision, ycoord1 double precision, xcoord2 double precision, ycoord2 double precision) 
...
$$
	...
	#route part 1
	string_route = r"CREATE TABLE route_part1 AS SELECT a.*, b.geom_way FROM pgr_astar('SELECT id as id, source::integer, target::integer, cost::double precision AS cost, reverse_cost::double precision AS reverse_cost, x1, y1, x2, y2 FROM osm_nl_2po_4pgr',{},{}) AS a LEFT JOIN osm_nl_2po_4pgr as b ON (node = source) ORDER BY seq".format(start1, parking1)
	con.execute(string_route)

	#route part 2
	string_route = r"CREATE TABLE route_part2 AS SELECT a.*, b.geom_way FROM pgr_astar('SELECT id as id, source::integer, target::integer, km::double precision AS cost, reverse_cost::double precision AS reverse_cost,x1,y1,x2,y2 FROM osm_nl_2po_4pgr',{},{})AS a LEFT JOIN osm_nl_2po_4pgr as b ON (node = source) ORDER BY seq".format(parking2, destination1)
	con.execute(string_route)
